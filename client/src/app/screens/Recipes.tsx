...
function ImportUrlSheet({
  ...
  async function submit(): Promise<void> {
    ...
    try {
      ...
      if (err instanceof MealsApiError && err.status >= 400 && err.status < 500) {
        ...
        setFieldError(safe && detail !== `HTTP ${err.status}` ? detail : t('recipes:import.invalid'));
      } else {
  ...
  return (
    ...
    >
      ...
      >
        <Field label={t('recipes:import.url')} {...(fieldError ? { error: fieldError } : {})} required>
          <Input
            ref={inputRef}
            type="url"
            inputMode="url"
        ...
        {fatalError ? (
          <p role="alert" className="font-body text-body text-rose-deep" data-testid="recipes-import-error">
            {fatalError}
          </p>
        ) : null}
