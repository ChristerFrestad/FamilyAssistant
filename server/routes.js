...
function registerRoutes(router, { repos, serverState }) {
  ...
  router.put('/api/meals/reorder', validateBody(schemas.mealsReorderBody), (ctx) => {
   ...
   *  - Testing / debugging
   */
  router.post('/api/profile/check-recipe', (ctx) => {
    const allergyFilter = require('./services/allergy-filter.service');
    const body = ctx.body || {};
  ...
  router.post('/api/recipes/import', validateBody(schemas.recipeImportTextBody), async (ctx) => {
  ...
  router.post('/api/recipes/import/image', async (ctx) => {
    const body = ctx.body || {};
  ...
  router.post('/api/shopping/add', validateBody(schemas.shoppingAddBody), (ctx) => {
    const wk = ensureCurrentWeek(repos);
   ...
   * force ikke er satt.
   */
  router.post('/api/shopping/generate', validateBody(schemas.shoppingGenerateBody), (ctx) => {
   ...
   */
  router.put('/api/shopping/items/:id/unpantry', (ctx) => {
    const itemId = parseInt(ctx.params.id, 10);
    if (!Number.isInteger(itemId) || itemId <= 0) throw errors.badRequest('Ugyldig id');
    const parent = repos.shoppingLists.getItemWithList(itemId);
    if (!parent) throw errors.notFound(`Item ${itemId} ikke funnet`);
   ...
   */
  router.post('/api/shopping/list/:id/enrich', (ctx) => {
    const id = parseInt(ctx.params.id, 10);
    if (!Number.isInteger(id) || id <= 0) throw errors.badRequest('Ugyldig id');
    const list = repos.shoppingLists.getById(id);
    if (!list) throw errors.notFound(`Handleliste ${id} ikke funnet`);
   ...
   */
  router.post('/api/shopping/list/:id/done', (ctx) => {
    const id = parseInt(ctx.params.id, 10);
    if (!Number.isInteger(id) || id <= 0) throw errors.badRequest('Ugyldig id');
    const list = repos.shoppingLists.getById(id);
    if (!list) throw errors.notFound(`Handleliste ${id} ikke funnet`);
  ...
  router.post('/api/consumables/:id/bought', validateBody(schemas.consumableBoughtBody), (ctx) => {
    const id = parseInt(ctx.params.id, 10);
  ...
  router.put('/api/pantry/correct', validateBody(schemas.pantryCorrectBody), (ctx) => {
    try {
  ...
  router.post('/api/settings/env', async (ctx) => {
    const envStore = require('./services/env-store.service');
  ...
  router.post('/api/integrations/:name/test', async (ctx) => {
    const envStore = require('./services/env-store.service');
  ...
  router.post('/api/receipts/upload', async (ctx) => {
  ...
  router.put('/api/receipts/confirm', validateBody(schemas.receiptConfirmBody), (ctx) => {
  ...
  router.post('/api/sunday-push/accept', validateBody(schemas.sundayAcceptBody), (ctx) => {
  ...
  router.post('/api/stt/transcribe', async (ctx) => {
  ...
  router.post('/api/llm/chat', validateBody(schemas.llmChatBody), async (ctx) => {
  ...
  router.post('/api/llm/recipe', validateBody(schemas.llmRecipeBody), async (ctx) => {
