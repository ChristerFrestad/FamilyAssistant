...
| Container shows **Running** but **Published Ports** is empty | Crash-loop: container exits before Docker publishes ports. **Most common:** data volume has a DB but no `AUTH_TOKEN` / `bootstrap.json` → log shows `AUTH_TOKEN er påkrevd`. Fix: open Logs, then either (a) set stack env `AUTH_TOKEN` + `ALLOWED_ORIGINS=http://<pi-ip>:7777` and redeploy, or (b) pull image ≥ recovery-bootstrap and open `/setup.html`, or (c) delete stack **with volumes** for a true fresh install (§16.5). Also check for OOM (`mem_limit`) and `address already in use`. |
