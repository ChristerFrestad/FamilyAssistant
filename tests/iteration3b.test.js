...
describe('Autogenerer-hook', () => {
  ...
  test('Andre oppdatering etter at lista finnes skal IKKE autogenerere på nytt', async () => {
    ...
    // Endre en dag via API
    const res = await request(server.baseUrl, 'PUT', '/api/meals/status', {
      body: { weekYear: wk, dayOfWeek: 0, status: 'skipped' },
    });
    assert.equal(res.status, 200);
    // Vi forventer at hooken ser at det allerede er en aktiv liste og hopper over
    assert.equal(res.body.autogeneratedShoppingList, null);

    const active = repos.shoppingLists.getActive(wk);
    assert.equal(active.id, first.listId); // samme liste, ikke ny
  });
});
