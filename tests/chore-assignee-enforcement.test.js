...
before(async () => {
  server = await startTestServer({ authToken: 'chore-assignee-enforcement-token-abcdef0123456789' });
});
...
test('child cannot complete a sibling assigned chore; own, unassigned, and adult can', async () => {
  ...
  assert.ok(todayMatch, `GET /api/today missing assigned chore: ${JSON.stringify(today.body.chores)}`);
