...
test('PATCH and DELETE of a foreign event id return 404', async () => {
  ...
  const still = server.repos._db.prepare('SELECT title FROM calendar_events WHERE id = ?').get(bEventId);
  assert.ok(still);
...
test('iCloud connect stores encrypted password and GET omits secrets', async () => {
  ...
  assert.equal(connect.status, 200, `iCloud connect ${connect.status} ${JSON.stringify(connect.body)}`);
