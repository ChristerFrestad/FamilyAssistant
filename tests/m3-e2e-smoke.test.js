...
describe('M3.1 · E2E smoke — forsiden', () => {
  ...
  test('GET /index.html serverer legacy v1 markup', async () => {
    ...
    // Week 3: modulariserte script-tags (<script src="/js/..."></script>) i
    // stedet for én stor inline <script>-blokk. Vi sjekker at minst én ekstern
    // JS-fil lastes, som bevis på at HTML-skallet peker på /js/-filene.
    assert.ok(/<script[^>]*src=["']\/js\//.test(res.raw));
  });
