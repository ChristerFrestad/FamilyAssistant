...
after(async () => {
  ...
  try { fs.rmSync(process.env.RECEIPTS_DIR, { recursive: true, force: true }); } catch { /* ignore */ }
});
...
describe('receipts repository', () => {
  ...
  test('updateParsed setter merchant og total', () => {
    ...
    const id = repos.receipts.insert({
      filePath: '/tmp/b.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 50, sha256: sha, status: 'pending',
    });
    repos.receipts.updateParsed(id, {
      merchant: 'Kiwi Vågsbygd',
    ...
    assert.equal(got.merchant, 'Kiwi Vågsbygd');
  ...
  test('receiptItems.insertMany + getByReceipt', () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/c.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha,
    });
  ...
  test('markStatus + stats', () => {
    ...
    const id = repos.receipts.insert({
      filePath: '/tmp/d.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha,
    });
...
describe('receipt.service utilities', () => {
  ...
  test('flagSuspiciousItem returnerer unknown_product for manglende key', () => {
    ...
    const flagged = flagSuspiciousItem(repos, {
      productKey: null, productName: 'Mystisk vare', qty: 1, totalPrice: 99,
    });
  ...
  test('flagSuspiciousItem flagger price_mismatch ved eksessiv pris', () => {
    ...
    const flagged = flagSuspiciousItem(repos, {
      productKey: 'ref-kaffe', productName: 'Kaffe', qty: 1, totalPrice: 200,
    });
...
describe('confirmReceipt', () => {
  test('oppdaterer inventory og markerer items confirmed', () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/x.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha,
      merchant: 'Rema 1000', purchasedAt: '2026-04-05', totalNok: 72.7,
      status: 'pending',
    });
    repos.receiptItems.insertMany(rid, [
      { lineText: 'Conf-test ost', productKey: 'conf-test-ost', productName: 'Ost 500g',
        qty: 1, unit: 'stk', totalPrice: 72.7, confidence: 0.9 },
    ]);
  ...
  test('hopper over items uten productKey', () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/y.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha, status: 'pending',
    });
    repos.receiptItems.insertMany(rid, [
  ...
  test('idempotent: repeat confirm returnerer alreadyConfirmed', () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/z.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha, status: 'pending',
    });
    repos.receiptItems.insertMany(rid, [
...
describe('HTTP: /api/receipts/*', () => {
  ...
  test('GET /api/receipts/:id returnerer receipt + items', async () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/http.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha, status: 'pending',
    });
    repos.receiptItems.insertMany(rid, [
  ...
  test('PUT /api/receipts/confirm oppdaterer inventory', async () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/httpconf.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha, status: 'pending',
    });
    repos.receiptItems.insertMany(rid, [
      { lineText: 'Conf-http', productKey: 'conf-http', productName: 'Conf', qty: 2, unit: 'stk', totalPrice: 40 },
    ]);
  ...
  test('DELETE /api/receipts/:id markerer rejected', async () => {
    ...
    const rid = repos.receipts.insert({
      filePath: '/tmp/del.jpg', mimeType: 'image/jpeg',
      fileSizeBytes: 10, sha256: sha, status: 'pending',
    });
